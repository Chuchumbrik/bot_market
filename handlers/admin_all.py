...
async def start_load_product(message: types.Message, state: FSMContext):
    if await Auth.Auth.check_access_level(message.from_user.username, message.from_user.id, state, "admin"):
        print("None")
...
async def del_call_back_run(callback_query: types.CallbackQuery):
    ...
    if check_exists_product:
        ...
        await delete_inline_button_callback(callback_query)
        await bot.send_message(callback_query.from_user.id, f'{name_product} удалена.',
                               reply_markup=admin_kb.kb_admin_global)
        await callback_query.answer()
    else:
        await bot.send_message(callback_query.from_user.id, "Продукт уже ранее удален",
                               reply_markup=admin_kb.kb_admin_global)
        await callback_query.answer()


async def delete_item(message: types.Message):
    if await Auth.Auth.check_access_level(message.from_user.username, message.from_user.id, None, "admin"):
        ...
        if check_exists_products:
            ...
            for item in catalog_product:
                ...
                await msg.edit_reply_markup(
                    InlineKeyboardMarkup().row(InlineKeyboardButton('Изменить', callback_data=f'edit {item[0]}'),\
                                               InlineKeyboardButton('Удалить', callback_data=f'del {item[0]}')))
            await bot.send_message(message.from_user.id,
                                   "Нажмите на кнопку 'Удалить' под продуктом, который хотите удалить", \
                                   reply_markup=admin_kb.kb_admin_global)
        else:
...
async def edit_product_main(state: FSMContext):
    ...
    if check_exists_product:
        ...
        await bot.send_message(id_user, 'Вы кто такие, я вас не знаю, идите нахуй!',
                               reply_markup=admin_kb.kb_admin_global)
...
async def cancel_callback_edit_product(callback_query: types.CallbackQuery, state: FSMContext):
    if await Auth.Auth.check_access_level(callback_query.from_user.username, callback_query.from_user.id, state, "admin"):
        ...
        if current_state is None:
            return
        await state.finish()
        await bot.send_message(callback_query.from_user.id, 'Редактирование завершено',
                               reply_markup=admin_kb.kb_admin_global)
        await callback_query.answer()
...
def register_handlers_admin(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(edit_product_photo, lambda x: x.data and x.data.startswith('edit_photo'),
    ...
    dp.register_callback_query_handler(edit_product_name, lambda x: x.data and x.data.startswith('edit_name'),
                                       state=FSMEditProducts.start)
    dp.register_message_handler(edit_product_name_state, state=FSMEditProducts.name)

    dp.register_callback_query_handler(edit_product_description,
                                       lambda x: x.data and x.data.startswith('edit_description'),
                                       state=FSMEditProducts.start)
    dp.register_message_handler(edit_product_description_state, state=FSMEditProducts.description)

    dp.register_callback_query_handler(edit_product_price, lambda x: x.data and x.data.startswith('edit_price'),
                                       state=FSMEditProducts.start)
